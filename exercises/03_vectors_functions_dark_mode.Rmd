...
```{r}
load("exercises/data_sets/hotdog.RData")
...
```{r}
summary(hotdog_2018, hotdog_2019)
```
...
```{r}
numbers_kseniya  <- 1:10
index <- numbers_kseniya > 6
numbers_kseniya [index]
```


4.6) Write code that replaces all elements larger than 6 in the vector `1:10` with the number 666.
```{r}
numbers_kseniya [index] <- c(666, 666, 666, 666)
numbers_kseniya [index] <- 666
```
...
```{r}
indexes = hotdog_2019 > hotdog_2018
improved <- hotdog_names [indexes]
indexes
improved
```
