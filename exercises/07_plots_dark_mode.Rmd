...
source("exercises/nerd_preprocessing.R")
...
```{r}
nerd_red %>% 
  ggplot(aes(x = age, y = nerd_score, shape= married, colour = married)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE)
```
...
```{r}
nerd_red [1:50, ] %>% 
  ggplot(aes(x = age, y = nerd_score , size= nerdy, colour = nerdy)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE)
```
...
Now we can use `seq()` to adjust our y axis like this:
...
nerd_red %>% 
  ggplot(aes(x = nerd_score, y = age)) +
  geom_point() +
  scale_y_continuous(breaks = seq(0, 130, 10), limits = c(10, 200))
```
...
nerd_red %>% 
  ggplot(aes(x = age, y = nerd_score)) +
  geom_point(size=4, alpha =.5, colour= "pink")
```
...
nerd_red %>% 
  ggplot(aes(x = gender, y = nerd_score, fill = married)) +
  scale_fill_manual(values = c("red", "blue", "purple")) + # custom colours
  scale_colour_manual(aesthetics = "orange", breaks = waiver(), na.value = "grey50")+
  geom_boxplot()
```
